...
{
    public static void Seed(EcoscolarDbContext context)
    {
        ...
        for (var i = 1; i <= 20; i++)
        {
            ...
            var userName = $"{firstName}.{lastName}".ToLowerInvariant().Replace(" ", string.Empty) + i;
            var email = $"{userName}@example.ch";
            var user = new User
            {
